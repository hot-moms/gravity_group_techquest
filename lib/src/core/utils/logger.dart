...
final class AppLogger$Logging extends AppLogger {
  ...
  static String _formatLoggerMessage({
    ...
    const canonicalPrefixLen = 19;
    final prefix = '[${logLevel.name}] ${time.formatTime()} ';

    final spaceFillings = canonicalPrefixLen - prefix.length;
    final spacing = ' ' * spaceFillings;
    return '$spacing [${logLevel.name}] ${time.formatTime()} | $message ${error != null ? '| $error' : ''}';
  }
